...
+myColor(X) <- print("I'm the ", X, " spaceman").

//receiving the initial room by the game system
+initialRoom(A)[source(gameManager)] <- +myRoom(A); -initialRoom(A)[source(gameManager)].

//when entering a new room, check if there is something to fix in the room
+myRoom(A) <- checkRoomSituation.
...
+wasFixing(A)[source(percept)] <- +trust(A); -untrust(A); -wasFixing(A)[source(percept)].

//if another player is seen killing in the same room, that player will be untrusted
+wasKilling(A)[source(percept)] <- +untrust(A); -trust(A); -wasKilling(A)[source(percept)].
...
+reportedImpostor(A,B,N) : ((trust(B) & not trust(A)) | (untrust(A) & not untrust(B))) & myColor(X) & not (A==X) & not(B==X) & not voted(A) & not expelled <-
...
+reportedImpostor(A,B,N) : ((trust(A) & not trust(B)) | (untrust(B) & not untrust(A))) & myColor(X) & not (A==X) & not voted(B) & not expelled <- 
...
+reportedImpostor(A,B,N) : trust(A) & trust(B) & not untrust(A) & not untrust(B) & myColor(X) & not (A==X) & not expelled <- 
...
+reportedImpostor(A,B,N) : not trust(A) & not trust(B) & not untrust(A) & not untrust(B) & myColor(X) & not (A==X) & not expelled <- 
													print("Sorry ", B, ", I'm not totally sure ", A, " is an impostor").
