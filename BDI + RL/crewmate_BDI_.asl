...
+wasFixing(A)[source(percept)] : not untrusted(A) <- +trusted(A); -wasFixing(A)[source(percept)].

//if another player is seen killing in the same room, that player will be untrusted
+wasKilling(A)[source(percept)] <- +untrusted(A); -trusted(A); -wasKilling(A)[source(percept)].
